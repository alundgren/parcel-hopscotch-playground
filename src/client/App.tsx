...
function TutorialCoach({ tutorial, onDismiss, disabled }: { tutorial: TutorialState; onDismiss: () => void; disabled: boolean }) {
  ...
  return <aside className={`tutorial-coach${targetAvailable ? "" : " tutorial-recovery"}`} id={targets.tutorialCoach} aria-label={`${tutorial.title} tutorial`} data-tutorial-id={tutorial.id} data-tutorial-phase={tutorial.phase} data-tutorial-step={tutorial.step}>
    {!targetAvailable && <p className="tutorial-recovery-copy">The next target is not available here. Return to Work or reopen the saved proposal or receipt.</p>}
...
function WorkQueue({ orders, latestReceipt, savedProposal, connected, selectedOrderId, selectOrder, prepare, prepareBatch, prepareReset, advance, openReceipt, openSavedProposal, selectReady, coach }: { orders: ReadonlyArray<OrderSummary>; latestReceipt: CommandReceipt | null; savedProposal: ReviewedProposal | null; connected: boolean; selectedOrderId: string | null; selectOrder: (id: string | null) => void; prepare: (id: string) => void; prepareBatch: () => void; prepareReset: () => void; advance: () => void; openReceipt: () => void; openSavedProposal: () => void; selectReady: () => void; coach: ReactNode }) {
...
export default function App() {
  ...
  const coach = snapshot?.tutorial === null || snapshot?.tutorial === undefined ? null : <TutorialCoach tutorial={snapshot.tutorial} onDismiss={dismissTutorial} disabled={busy || status !== "connected"} />;
