...
import { resolutionFor, type InventoryCondition, type ResolutionPolicy } from "./fulfilment.js";
...
interface PreparedPolicy extends ResolutionPolicy { readonly priorStatus: OrderStatus; readonly priorIssue: string; readonly priorCompleted: number; readonly nextCompleted: number }
...
const seedUser = (database: DatabaseSync, userId: string) => {
  ...
  if (Number(count.count) === 0) {
    const order = database.prepare("INSERT INTO orders (user_id, order_id, item, issue, status, family, seed_position, version) VALUES (?, ?, ?, ?, ?, ?, ?, 1)");
    const evidence = database.prepare("INSERT INTO order_evidence (user_id, order_id, label, value, occurred_at) VALUES (?, ?, ?, ?, ?)");
    seedOrders.forEach((item, position) => { order.run(userId, item.id, item.item, item.issue, item.status, item.family, position); evidence.run(userId, item.id, item.evidenceLabel, item.evidenceValue, item.occurredAt); });
  }
...
const readSnapshot = (database: DatabaseSync, identity: RequestIdentity, now = Date.now()): WorkspaceSnapshot => {
  ...
  const orders = database.prepare(`SELECT order_id, item, issue, status, family, version FROM orders WHERE user_id = ? AND completed = 0 ORDER BY CASE order_id WHEN 'BB-1051' THEN 0 WHEN 'BB-1063' THEN 1 WHEN 'BB-1042' THEN 2 WHEN 'BB-1088' THEN 3 ELSE 4 END, seed_position`).all(user.id) as Array<{ order_id: string; item: string; issue: string; status: OrderStatus; family: WorkspaceSnapshot["orders"][number]["family"]; version: number }>;
...
const makePublic = (generation: number, kind: ReviewedProposal["kind"], title: string, policies: ReadonlyArray<PreparedPolicy>, omissions: ReviewedProposal["omissions"], effects: ReadonlyArray<string>): ReviewedProposal => ({ id: `proposal_${randomUUID()}`, generation, kind, title, ready: true, changes: policies.map((policy) => policy.change), omissions, effects, createdAt: new Date().toISOString() });
const preparePolicy = (database: DatabaseSync, userId: string, orderId: string): PreparedPolicy => {
  const row = rowFor(database, userId, orderId);
  if (row === undefined) throw fail("order_not_found", "That order is not in this workspace.");
  if (Number(row.completed) !== 0) throw fail("order_completed", "That order already moved to packing.");
  const base = resolutionFor(row, row.family === "substitution" ? inventoryVersion(database, userId, "MUG-SAGE") : row.family === "bundle" ? inventoryVersion(database, userId, "SIDE-PLATE") : undefined);
  return { ...base, priorStatus: row.status, priorIssue: row.issue, priorCompleted: Number(row.completed), nextCompleted: Number(row.completed) };
};
...
const repositoryLayer = (filename: string) => Layer.effect(WorkspaceRepository, Effect.acquireRelease(
  ...
  const prepareResolution: WorkspaceRepositoryService["prepareResolution"] = (identity, generation, orderId) => command(() => transact(database, () => {
    ...
    const policy = preparePolicy(database, user.id, orderId);
    const publicProposal = makePublic(generation, "resolution", policy.change.family === "address" ? "Check address" : `Review ${orderId}`, [policy], [], policy.inventory === null ? [policy.change.effect] : [policy.change.effect, `Stock ${policy.inventory.sku} version ${policy.inventory.expectedVersion} will be checked again.`]);
    return storeProposal(database, user.id, { public: publicProposal, policies: [policy] });
  }));
  const prepareBatch: WorkspaceRepositoryService["prepareBatch"] = (identity, generation) => command(() => transact(database, () => {
    const user = ensureUser(database, identity); expectGeneration(user.generation, generation);
    const rows = database.prepare("SELECT order_id, item, issue, status, family, version, completed FROM orders WHERE user_id = ? ORDER BY seed_position").all(user.id) as Array<{ order_id: string; item: string; issue: string; status: OrderStatus; family: WorkspaceSnapshot["orders"][number]["family"]; version: number; completed: number }>;
    ...
    const policies: Array<PreparedPolicy> = included.map((row) => ({ change: { orderId: row.order_id, family: row.family, before: `${row.item} · Ready`, after: `${row.item} · Packing`, effect: "Release this checked order to packing.", expectedVersion: Number(row.version) }, nextStatus: "ready", nextIssue: "Released to packing after review.", inventory: null, priorStatus: row.status, priorIssue: row.issue, priorCompleted: Number(row.completed), nextCompleted: 1 }));
  ...
  const prepareUndo: WorkspaceRepositoryService["prepareUndo"] = (identity, generation, receiptId) => command(() => transact(database, () => {
    ...
    const policies = receipt.applied.map((applied): PreparedPolicy => {
      ...
      const inventory = applied.inventory === null ? null : { ...applied.inventory, expectedVersion: inventoryVersion(database, user.id, applied.inventory.sku), quantity: -applied.inventory.quantity };
      return { change: { orderId: applied.change.orderId, family: applied.change.family, before: applied.change.after, after: applied.change.before, effect: `Reverse: ${applied.change.effect}`, expectedVersion: applied.committedVersion }, nextStatus: applied.priorStatus, nextIssue: applied.priorIssue, inventory, priorStatus: current.status, priorIssue: current.issue, priorCompleted: Number(current.completed), nextCompleted: applied.priorCompleted };
    });
  ...
  const accept: WorkspaceRepositoryService["accept"] = (identity, generation, proposalId, idempotencyKey) => command(() => transact(database, () => {
    ...
    for (const policy of stored.policies) {
      ...
      if (policy.inventory !== null) {
        const stock = database.prepare("SELECT quantity, version FROM inventory WHERE user_id = ? AND sku = ?").get(user.id, policy.inventory.sku) as { quantity: number; version: number } | undefined;
        if (stock === undefined || Number(stock.version) !== policy.inventory.expectedVersion || Number(stock.quantity) < policy.inventory.quantity) throw fail("stale_proposal", `Stock for ${policy.inventory.sku} changed after review. Nothing was applied.`);
      }
    }
    const applied: Array<AppliedChange> = [];
    for (const policy of stored.policies) {
      database.prepare("UPDATE orders SET status = ?, issue = ?, completed = ?, version = version + 1 WHERE user_id = ? AND order_id = ?").run(policy.nextStatus, policy.nextIssue, policy.nextCompleted, user.id, policy.change.orderId);
  ...
  const advanceScenario: WorkspaceRepositoryService["advanceScenario"] = (identity, generation) => command(() => transact(database, () => {
    ...
    database.prepare("UPDATE orders SET issue = 'Scenario stock changed. Review the replacement again.', status = 'review', version = version + 1 WHERE user_id = ? AND order_id = 'BB-1051'").run(user.id);
    appendEvent(database, user.id, generation, "scenario.stock_changed", { sku: "MUG-SAGE", quantity: Number(stock.quantity) - 1 });
    return { message: "Scenario advanced: sage mug stock and BB-1051 changed.", snapshot: readSnapshot(database, identity) };
  }));
