...
Piploy builds the root `Dockerfile` with the repository root as its context and creates one container. The application build and fresh production-dependency stages use `ghcr.io/voidzero-dev/vite-plus:0.3.0@sha256:bca24ac970b21298430ad281f306dbe0a17be3fd1d6c9ec5f2cc73da65740b88` on `$BUILDPLATFORM`. The runtime uses `node:24-bookworm-slim@sha256:3638d9a6fe4030bd716be989438248074489337ba3275657f93595428be4fc03`. Both are multi-platform OCI index references. Literal references matter because the current Piploy parser rejects unresolved Dockerfile variables in image references.

The production-dependency stage starts without the development install and runs `vp install --prod --frozen-lockfile`. Vite Plus 0.3.0 does not support `vp pm deploy`, and installing with `--prod` over a full dependency tree does not remove packages already present. The current production graph has no native addon or target-dependent binary, so its builder-platform install can be copied into either selected runtime architecture. Recheck the final install and install for the target architecture if that graph changes.
...
The selected indexes contain `linux/amd64` and `linux/arm64/v8` manifests. For ARM64, the builder resolves to manifest `sha256:724bc05c3229dd1784a8227484e18eb2596e5ac23f54edab3afab4560cd021e6` and the Node runtime resolves to `sha256:c133efe216ffb6e785ed9a8be55a29fcb86775e8008ae0a9f0ed6af4f175bb03`. They share compressed layer `sha256:0f5d7465a5bb9d419f60c93d126a161286c73a1ede4a8b2e46bd5e7ad5782cc7`, whose unpacked diff ID is `sha256:709e0d49f62206f760d5392af4f1dab26c0017f4242f3975ffe0b2ce705f42e6`.

Registry layer sizes describe compressed transfer bytes. They do not establish local disk use, and build cache is separate from layers retained by a running image. No production-host inventory was taken. Actual Pi image presence, shared-layer reuse and incremental savings remain unknown until the owner checks them during the deployment audit.
