...
`vp run test:qa` checks the exploratory QA records, role separation, memory,
accounting, and command wrapper in Node without a build or browser. App-specific
QA tests use the existing Vitest unit and integration project. The portable
`tools/qa-loop` tests use Node's built-in runner so the copied module can verify
its contract without installing this app's dependencies. They run separately as
`vp run test:qa:core`. Every test removes its disposable files during teardown.

`vp run test:qa:e2e` builds the app and uses Playwright through the actual QA
adapter to check its browser commands, saved state, spending gate, and shutdown.
It has its own temporary database and loopback port. This check proves the QA
adapter's connection to the app; component UI states remain in Browser Mode.
CI runs the Node QA checks in the main check job and the adapter lifecycle in
the browser job. The adapter records its own Playwright context during this
offline check. Passing tests discard the trace; failures retain it and app logs
under ignored `artifacts/qa-adapter-failures/` for CI upload. Neither command uses
live inference.
