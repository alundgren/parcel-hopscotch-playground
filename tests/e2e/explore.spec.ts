...
test("filters and inspects the validated registry catalogue", async ({ page, context }, testInfo: TestInfo) => {
  ...
  await capture(page, testInfo, `actual-explore-default-${testInfo.project.name}.png`);
  ...
  await capture(page, testInfo, `actual-explore-expanded-${testInfo.project.name}.png`);
});
...
test("launches the orientation, tutorial, and batch scenarios through working flows", async ({ page }) => {
  ...
  await expect(page.getByText("Nothing changes until you accept it in the app.")).toBeVisible();
  await expect(page.getByText("Accepted by you")).toHaveCount(0);
  await page.getByLabel(/Review \d+ changes/).getByRole("button", { name: "Cancel" }).click();
});
