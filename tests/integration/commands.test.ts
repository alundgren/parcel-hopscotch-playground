...
describe("reviewed fulfilment commands", () => {
  ...
  it("reads the latest order receipt through resolution, packing, and Undo without crossing users", async () => {
    ...
    await useRepository(filename, (repository) => Effect.gen(function* () {
      ...
      expect(resolved).toMatchObject({ order: { status: "ready", version: 2, businessValue: "Sage stoneware mug, quantity 1, £24.00" }, completed: false, resolved: true, latestReceipt: { id: accepted.receipt.id, kind: "accept", totalChanges: 1, change: { before: "Blue stoneware mug, quantity 1, £24.00", after: "Sage stoneware mug, quantity 1, £24.00" } } });
      ...
      expect(completed).toMatchObject({ order: { version: 3, businessValue: "Sage stoneware mug, quantity 1, £24.00" }, completed: true, resolved: true, latestReceipt: { id: packed.receipt.id, kind: "accept", totalChanges: 6, change: { orderId: "BB-1051", after: "Sage stoneware mug, quantity 1, £24.00 · Packing" } } });
      ...
      expect(yield* repository.orderProgress(other, 1, "BB-1051")).toMatchObject({ order: { version: 3, businessValue: "Blue stoneware mug, quantity 1, £24.00" }, completed: false, resolved: false, latestReceipt: { id: undone.id, kind: "undo", change: { after: "Blue stoneware mug, quantity 1, £24.00" } } });
    }));
    const database = new DatabaseSync(filename);
